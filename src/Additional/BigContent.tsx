    ...
    return (
        <div className='grid bg-gray-800/80 fixed top-0 left-0 h-full w-full justify-items-center items-center' key={fullData.mal_id}>
            <div className='grid grid-cols-2 bg-black h-1/2 w-1/2 rounded-3xl'>
                ...
                <div className='grid grid-rows-[1.5fr_1.5fr_10fr_1fr_1fr]  items-center  text-white text-sm h-[459px] mx-6'>
                    <div className='grid grid-cols-[3fr_1fr] items-center' >
                        <div className='grid text-lg '> {fullData.title} </div>
                        <div className='grid justify-self-end text-base'> Rating:{fullData.score}({fullData.scored_by})</div>
                    </div>
                    <div className='grid justify-self-end text-lg over'>{fullData.title_japanese}</div>
                    <div className='grid max-h-[306px] overflow-y-auto self-start'>{fullData.synopsis}</div>
                    ...
                    <a href={fullData.url} className='grid justify-self-end'>Website: {fullData.url}</a>
                </div>
            </div>
        </div>)
